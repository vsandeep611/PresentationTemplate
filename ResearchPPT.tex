		...
		\begin{column}{0.5\textwidth}
			\begin{figure}[!h]
				\begin{adjustbox}{max totalsize={.99\textwidth}{.5\textheight},center}
					...
					(S_2) edge              node[left]  {r3 [$t_1\ge2$]}  (S_4)
					(S_3) edge              node[right] {r4 [$t_1\ge2$]}  (S_5)
					(S_4) edge              node[left]  {r5}  (S_6)
					(S_5) edge              node[right] {r6}  (S_6)
...
%\begin{frame}{Clock Allocation Algorithm Overview}
%	
%	
%\end{frame}
...
\begin{frame}{Light Control System}
	Consider the variant of the light control system \cite{TimedAutomata:Patricia}:
	\begin{enumerate}
...
Consider the behaviour of a traffic light \cite{Dubey2007}. 
	\begin{itemize}
...
\begin{frame}{CSMA/CD Protocol}
	Consider a variant of the CSMA/CD protocol \cite{CSMA/CD}.
	\begin{figure}
		\begin{adjustbox}{max totalsize={.99\textwidth}{.75\textheight},center}	
