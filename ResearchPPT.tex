...
\begin{figure}
	\begin{adjustbox}{max totalsize={.99\textwidth}{.8\textheight},center}
		...
		\path 	(S1)   edge          		node[anchor=south, above] {t}     (S2)
				...
				(S1)   edge          		node[anchor=south, above] {not\_t}     (S2)
				(S2)   edge                 node[anchor=south, above] {not\_i}	  (S3)
				(S3)   edge                 node[anchor=south, above] {not\_m} 	  (S4)
				(S4)   edge   				node[anchor=south, above] {not\_e}     (S5)   
		;
		\end{tikzpicture}
		\end{adjustbox}
		\caption{Simple finite state automaton}
		\label{fig:FSA}		
	\end{figure}
